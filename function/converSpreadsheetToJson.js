const convertSpreadsheetToJson = {
          ...
          if (map.column >= 0 && map.column < row.length) {
            ...
            if (mergedFields.includes(map.field)) {
              if (
                ...
                // Jika kosong, gunakan lastMergedValues (jika ada)
                // JANGAN set ke obj jika lastMergedValues masih null
                if (lastMergedValues[map.field] !== null) {
                  obj[map.field] = lastMergedValues[map.field];
                }
                // Jika lastMergedValues null, field tidak ditambahkan ke obj
              }
            } else {
            ...
            // Index tidak valid atau tidak ada
            if (mergedFields.includes(map.field)) {
              // Untuk merged field, gunakan nilai terakhir jika ada
              if (lastMergedValues[map.field] !== null) {
                obj[map.field] = lastMergedValues[map.field];
              }
              // Jika tidak ada, field tidak ditambahkan
            } else {
              obj[map.field] = "-";
            }
          }
  ...
  convertSpreadsheetToJSONNotUse(
    data,
    indexStartData,
    mapping,
    mergedFields = []
